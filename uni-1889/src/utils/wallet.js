...
export function channelIconUrl(ch) {
  ...
  if (icon.startsWith('/1889/')) {
    const rel = icon.replace(/^\/1889\//, '')
  ...
  if (icon.startsWith('/888/')) {
    // 历史路径：优先映射到 /1889 或 /assets，避免继续依赖废弃 /888
    const rest = icon.replace(/^\/888\//, '')
    return publicAbs('/1889/' + rest) || publicAbs('/assets/' + rest) || publicAbs(icon)
  }
