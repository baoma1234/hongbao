...
function normalizeStickerUrl(url) {
  ...
  let rel = ''
  if (s.indexOf('/1889/static/stickers/') === 0) {
    rel = s.slice('/1889/static/'.length)
  ...
  // 内置 wechat 表情包：App 用打包本地 /static；H5 用 /1889/static
...
/** 发给 IM 的 sticker url：只允许 /1889/static/stickers（历史 /888 入站 remap 到 999） */
function stickerSendUrl(url) {
  ...
  // 打包本地 /static/stickers → 服务端 allow 的 /1889/static/stickers
  ...
  if (s.indexOf('/888/stickers/') === 0) {
    return '/1889/static/stickers/' + s.slice('/888/stickers/'.length)
  }
  if (s.indexOf('/888/static/stickers/') === 0) {
    return '/1889/static/stickers/' + s.slice('/888/static/stickers/'.length)
  }
  if (s.indexOf('static/stickers/') === 0) {
    return '/1889/' + s
  }
  if (s.indexOf('stickers/') === 0) {
    return '/1889/static/' + s
  }
  if (
...
async function loadStickers() {
  ...
  try {
    ...
    const stickerUrl = 'https://shd12h.ykjab.com/1889/static/data/stickers.json'
...
async function sendSticker(st) {
  ...
  const payload = {
    ...
    extra: {
      ...
      code,
      // url 固定 /1889/static/stickers；fullurl 用展示地址（App 可走打包 static）
      url: sendUrl,
