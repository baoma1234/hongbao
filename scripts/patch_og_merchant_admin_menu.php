...
ensureMenu($pdo, $insert, $rule, $ogId, 'fanshub/ogmerchant/testdeposit', '测试存入', 'fa fa-circle-o', 0, 0, $now);
ensureMenu($pdo, $insert, $rule, $ogId, 'fanshub/ogmerchant/testwithdraw', '测试提出', 'fa fa-circle-o', 0, 0, $now);
ensureMenu($pdo, $insert, $rule, $ogId, 'fanshub/ogmerchant/testhistory', '转账历史', 'fa fa-circle-o', 0, 0, $now);
...
ensureMenu($pdo, $insert, $rule, $ogId, 'fanshub/ogmerchant/testbethistory', '投注记录', 'fa fa-circle-o', 0, 0, $now);
...
if ($group) {
    ...
    $need = $pdo->query(
        "SELECT id FROM {$rule} WHERE name IN (
            ...
            'fanshub/ogmerchant/testbethistory'
        )"
