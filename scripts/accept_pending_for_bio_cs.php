 ...
 *
 * 表字段对齐 ContactService：status / handle_user_id / updatetime（无 handled_by / handletime）
 */
...
// 确认账号存在
$u = $pdo->query("SELECT id,nickname,mobile FROM {$p}user WHERE id={$bioId}")->fetch(PDO::FETCH_ASSOC);
if (!$u) {
    fwrite(STDERR, "BIO user {$bioId} not found\n");
    exit(1);
}
echo 'BIO=' . json_encode($u, JSON_UNESCAPED_UNICODE) . "\n";
