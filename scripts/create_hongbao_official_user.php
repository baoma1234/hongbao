...
$avatarRel = '';
$srcCandidates = [
    $root . '/public/uploads/avatars/88888888',
    $root . '/uni-999/src/static/logo.png',
    $root . '/public/999/static/logo.png',
];
foreach ($srcCandidates as $c) {
    if (is_dir($c)) {
        $files = glob($c . '/*.{png,jpg,jpeg,webp}', GLOB_BRACE) ?: [];
        if ($files) {
...
echo "NOTE: multi-login via multi_login_user_ids; notice auto-approve via notice_auto_approve_user_ids\n";
