...
/**
 * 安装充值列表 / 提现列表菜单
 * php scripts/install_wallet_order_menu.php
 */
...
$menus = [
    [
        'name'  => 'fanshub/rechargeorder',
        'title' => '充值列表',
        'icon'  => 'fa fa-plus-circle',
        'weigh' => 74,
        'children' => [
            ['fanshub/rechargeorder/index', '查看'],
            ['fanshub/rechargeorder/markpaid', '确认到账'],
            ['fanshub/rechargeorder/markfailed', '作废'],
        ],
    ],
    [
        'name'  => 'fanshub/withdraworder',
        'title' => '提现列表',
        'icon'  => 'fa fa-minus-circle',
        'weigh' => 73,
        'children' => [
            ['fanshub/withdraworder/index', '查看'],
            ['fanshub/withdraworder/markpaid', '确认打款'],
            ['fanshub/withdraworder/reject', '拒绝退回'],
        ],
    ],
];
...
$extra = $pdo->query("SELECT id FROM {$rule} WHERE name LIKE 'fanshub/rechargeorder%' OR name LIKE 'fanshub/withdraworder%'")->fetchAll(PDO::FETCH_COLUMN);
...
echo "DONE 请刷新后台（Ctrl+F5）。菜单：福利大厅 → 充值列表 / 提现列表\n";
